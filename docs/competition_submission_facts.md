...
**상권 위험 지도 시각화 기능**
BIZ PATH는 서울 상권을 지도 위에 표시하고, 각 상권의 위험도와 우선순위를 색상과 크기로 표현합니다. 상권을 클릭하면 해당 상권의 GRI 점수, 유동인구, 폐업률, 순유입·순유출, 네트워크 중심성 등 주요 지표를 상세 패널에서 확인할 수 있습니다. MapLibre와 Deck.gl을 활용해 상권 노드, 행정동 경계, 상권 경계, 이동 흐름, 위험 연결선을 시각화합니다.

**인구 이동 흐름 기반 위험 탐지 기능**
수도권 생활이동 OD 데이터를 행정동 단위에서 상권 단위로 변환해 상권 간 이동 네트워크를 구성합니다. NetworkX 기반 유향 그래프를 사용해 상권별 `net_flow`, `in_degree`, `out_degree`, `degree_centrality`를 계산합니다. 이를 통해 단순 매출 지표만으로는 보이지 않는 유입 감소, 유출 증가, 연결성 약화 같은 흐름 기반 위험 신호를 탐지합니다.

**GRI 상권 위험 지수 산출 기능**
상권별 폐업률, 순유출, 네트워크 고립도를 결합해 0~100점의 GRI 위험지수를 산출합니다. 점수가 높을수록 침체·폐업·고립 위험이 높은 상권으로 해석합니다. GRI는 z-score 정규화와 백분위 점수화를 거쳐 상권 간 비교가 가능하도록 구성했습니다.

**AI 창업 입지 추천 기능**
사용자가 창업 희망 업종을 선택하면, 서비스가 상권별 GRI, 유동인구, 업종별 폐업률, 점포 수, 네트워크 중심성을 종합해 상권을 평가합니다. 평가 결과를 추천·주의·비추천 3단계로 분류해 창업자가 진입하기 좋은 상권과 피해야 할 상권을 보여줍니다. Anthropic Claude API를 활용하여 추천 상권별 이유와 주의사항을 자연어로 생성해 제공합니다.

**정책 추천 카드 자동 생성 기능**
상권 상태에 따라 지자체가 참고할 수 있는 정책 대응 카드를 자동 생성합니다. 유동인구 변화, 폐업률 상승, 상권 고립, 흐름 단절 등의 조건에 따라 임대료 안정, 유동인구 유도, 긴급 대응, 진입 관리 같은 정책 유형을 매칭합니다. 정책 카드는 생성형 AI가 아닌 규칙 기반 엔진으로 생성되므로 환각 위험이 없고, 감사나 보고 과정에서 의사결정 근거를 추적할 수 있습니다.

**흐름 단절 구간 시각화 기능**
상권 간 이동 흐름이 약해지거나 단절 위험이 큰 구간을 지도 위에 선과 애니메이션으로 표시합니다. 이를 통해 "어느 상권이 위험한가"뿐 아니라 "어디와의 연결이 약해졌는가"를 직관적으로 확인할 수 있습니다. 단순 직선 연결이 아니라 실제 도로 경로 기반 시각화를 지원하며, 외부 경로 API 장애 시에도 정적 캐시와 fallback 데이터로 데모가 가능하도록 설계했습니다.

**3D 상권 시각화 기능**
Deck.gl 기반 3D 뷰를 제공하며, 사용자가 상권 3D 모드를 켜면 지도가 45도 기울어진 입체 시점으로 전환됩니다. 3D 모드에서는 `CommerceColumnLayer`가 상권 중심 위치에 원기둥을 표시합니다. 사용자는 높이와 색상 기준 지표를 상권 위험도(GRI), 순유입 인구, 폐업률, 연결 중심성 중에서 선택할 수 있습니다.

### 4-2. 상권 위험 진단과 정책 카드 기능
...
> BIZ PATH는 예비 창업자가 무리한 입점과 조기 폐업을 피할 수 있도록 상권별 유입·유출 흐름, 폐업률, 네트워크 고립도, GRI 위험지수, 정책 우선순위를 한 화면에서 제공하는 상권 위험 진단 서비스입니다. 서울 상권을 지도 위에 표시하고 각 상권의 위험도와 우선순위를 색상과 크기로 표현하며, 상권을 클릭하면 GRI 점수, 유동인구, 폐업률, 순유입·순유출, 네트워크 중심성 등 주요 지표를 상세 패널에서 확인할 수 있도록 구현했습니다. MapLibre와 Deck.gl을 활용해 상권 노드, 행정동 경계, 상권 경계, 이동 흐름, 위험 연결선을 시각화합니다.
>
> 인구 이동 흐름 기반 위험 탐지 기능은 수도권 생활이동 OD 데이터를 행정동 단위에서 상권 단위로 변환해 상권 간 이동 네트워크를 구성합니다. NetworkX 기반 유향 그래프를 사용해 상권별 `net_flow`, `in_degree`, `out_degree`, `degree_centrality`를 계산하고, 단순 매출 지표만으로는 보이지 않는 유입 감소, 유출 증가, 연결성 약화 같은 흐름 기반 위험 신호를 탐지합니다. GRI 상권 위험 지수는 상권별 폐업률, 순유출, 네트워크 고립도를 결합해 0~100점으로 산출하며, z-score 정규화와 백분위 점수화를 거쳐 상권 간 비교가 가능하도록 구성했습니다.
>
> AI 창업 입지 추천 기능은 사용자가 창업 희망 업종을 선택하면 상권별 GRI, 유동인구, 업종별 폐업률, 점포 수, 네트워크 중심성을 종합해 상권을 평가합니다. 평가 결과를 추천·주의·비추천 3단계로 분류해 창업자가 진입하기 좋은 상권과 피해야 할 상권을 보여주며, Anthropic Claude API를 활용하여 추천 상권별 이유와 주의사항을 자연어로 생성해 제공합니다. 업종별 추천·주의·비추천 3단계 결과는 매출 규모나 임대 조건만 보고 결정하는 위험을 줄이고, CSV 출력을 통해 가족이나 상담사와 함께 검토하는 과정까지 이어질 수 있도록 구현했습니다.
>
> 정책 추천은 R4~R7 규칙 기반 PolicyCard로 구현했습니다. R4는 `흡수형_과열` 및 GRI 70 이상 상권에 젠트리피케이션 예방 정책을 제안하고, R5는 GRI 50~70 과열 초기 상권에 업종 균형과 프랜차이즈 비율 모니터링을 제안합니다. R6는 성장형 저위험 상권에 업종 박람회·소셜 마케팅 연계 등 성장 지원을 제안하며, R7은 안정형 저위험 상권을 정기 모니터링 대상으로 관리합니다. 정책 카드 본문은 생성형 AI가 아니라 코드화된 규칙으로 생성되어 같은 입력에는 같은 권고가 나오며, 각 카드에는 GRI·순유입·폐업률 등 발동 근거가 함께 표시됩니다. 이를 통해 담당자의 수기 보고 시간을 줄이고, 수치 근거가 포함된 정책 후보 목록을 빠르게 만들 수 있습니다.
>
> 흐름 단절 구간 시각화 기능은 상권 간 이동 흐름이 약해지거나 단절 위험이 큰 구간을 지도 위에 선과 애니메이션으로 표시합니다. 이를 통해 "어느 상권이 위험한가"뿐 아니라 "어디와의 연결이 약해졌는가"를 직관적으로 확인할 수 있습니다. 단순 직선 연결이 아니라 실제 도로 경로 기반 시각화를 지원하며, 외부 경로 API 장애 시에도 정적 캐시와 fallback 데이터로 데모가 가능하도록 설계했습니다. 3D 상권 시각화 기능은 Deck.gl 기반 3D 뷰를 제공하며, 상권 3D 모드에서 `CommerceColumnLayer`가 상권 중심 위치에 원기둥을 표시합니다. 사용자는 높이와 색상 기준 지표를 상권 위험도(GRI), 순유입 인구, 폐업률, 연결 중심성 중에서 선택할 수 있습니다.
>
> 검증 보고 탭은 5개 검증 항목을 함께 제공합니다. H1은 "GRI 상위 20% 상권의 다음 분기 폐업률이 하위 80%보다 높은가", H2는 "상권 순유출(`net_flow < 0`)이 클수록 폐업률 상승과 양의 상관을 보이는가", H3는 "네트워크 고립도(`degree_centrality` 하위)가 높을수록 매출 감소와 폐업률 상승이 동반되는가"를 검증합니다. B1은 "기존 공식 지표가 놓친 위험 상권을 GRI가 추가로 식별하는가", B3는 "단순 매출 추세 모델이 놓친 위험 신호를 GRI가 추가로 포착하는가"를 비교합니다.
>
> H1 실측 결과 2025년 3분기 GRI 상위 20% 상권의 2025년 4분기 평균 폐업률은 1.084%였고, 하위 80%는 0.075%로 약 14.5배 차이(gap=1.009%p, p≈8.0e-31, n=1,650)를 확인했습니다. 이는 GRI가 다음 분기 폐업 위험을 사전에 가리키는 정책 선별 지표로 활용될 수 있음을 보여줍니다. H2는 Spearman 상관과 분위 비교로 순유출과 폐업률 상승의 관계를 노출하고, H3는 네트워크 고립도가 높은 상권에서 매출 감소와 폐업률 상승이 함께 나타나는지 분위별로 비교합니다.
>
> 베이스라인 비교에서는 B1·B3 결과를 표로 제시합니다. B1은 기존 공식 지표가 놓친 추가 위험 상권 187건을 비교하고, B3는 매출 추세 모델 대비 추가 위험 신호 231건을 보완적으로 제시합니다. 검증 화면에는 5개 검증 항목(H1·H2·H3, B1·B3) 비교 결과가 공개되어 분석의 한계와 강점을 함께 확인할 수 있으며, BIZ PATH가 기존 지표를 보완해 흐름 기반 위험을 조기에 발견하고 창업자·지원기관·자치구가 사전 대응할 수 있는 근거를 제공합니다.
...
● BIZ PATH는 예비 창업자가 무리한 입점과 조기 폐업을 피할 수 있도록 수치 기반 판단 근거를 제공합니다. 또한, 업종별 추천·주의·비추천 3단계 결과는 매출 규모나 임대 조건만 보고 결정하는 위험을 줄이고, CSV 출력을 통해 가족이나 상담사와 함께 검토하는 과정까지 이어질 수 있습니다.

● 이는 창업 초기 잘못된 입지 선택에서 비롯되는 실패 비용을 줄이는 효과를 기대할 수 있습니다.

● 행정 측면에서는 폐업 발생 후 대응하는 방식에서, 위험 신호를 먼저 발견하고 개입하는 방식으로 전환하는 효과를 기대할 수 있습니다. 실제 검증 결과 2025년 3분기 GRI 상위 20% 상권의 2025년 4분기 평균 폐업률은 **1.084%**로, 하위 80%의 **0.075%** 대비 약 **14.5배** 높았습니다(gap=1.009%p, p≈8.0e-31, n=1,650).

● 이는 GRI가 다음 분기 폐업 위험을 사전에 가리키는 정책 선별 지표로 활용될 수 있음을 보여줍니다.

● 규칙 기반 정책 카드(R4~R7)와 CSV 출력 기능을 통해 담당자의 수기 보고 시간을 줄이고, 수치 근거가 포함된 정책 후보 목록을 빠르게 만들 수 있습니다.

● 정책 카드 본문은 생성형 AI가 아니라 규칙 기반으로 생성되어 환각 위험을 낮추고, GRI·순유입·폐업률 등 발동 지표를 함께 제공해 설명 가능성을 높입니다.

● 검증 화면에는 5개 검증 항목(H1·H2·H3, B1·B3) 비교 결과가 공개되어 분석의 한계와 강점을 함께 확인할 수 있으며, 기존 공식 지표가 놓친 추가 위험 상권 **187건**과, 매출 추세 모델 대비 추가 위험 신호 **231건**을 보완적으로 제시합니다.
...
> ● 정책 카드 본문은 생성형 AI가 아니라 규칙 기반으로 생성되어 환각 위험을 낮추고, GRI·순유입·폐업률 등 발동 지표를 함께 제공해 설명 가능성을 높입니다.
>
> ● 검증 화면에는 5개 검증 항목(H1·H2·H3, B1·B3) 비교 결과가 공개되어 분석의 한계와 강점을 함께 확인할 수 있으며, 기존 공식 지표가 놓친 추가 위험 상권 187건과, 매출 추세 모델 대비 추가 위험 신호 231건을 보완적으로 제시합니다.
