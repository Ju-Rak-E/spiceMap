...
> spiceMap은 상권별 유입·유출 흐름, 폐업률, 네트워크 고립도를 결합해 GRI 위험지수와 정책 우선순위를 산출하고, 지도·검증 보고·CSV 출력·창업 입지 추천으로 연결한 상권 위험 진단 서비스이다. 정책 추천은 R4~R7 규칙 기반 PolicyCard로 구현했다. R4는 `흡수형_과열` 및 GRI 70 이상 상권에 젠트리피케이션 예방 정책을 제안하고, R5는 GRI 50~70 과열 초기 상권에 업종 균형과 프랜차이즈 비율 모니터링을 제안한다. R6는 성장형 저위험 상권에 업종 박람회·소셜 마케팅 연계 등 성장 지원을 제안하며, R7은 안정형 저위험 상권을 정기 모니터링 대상으로 관리한다. 정책 카드 본문은 생성형 AI가 아니라 코드화된 규칙으로 생성되어 같은 입력에는 같은 권고가 나오며, 각 카드에는 GRI·순유출·폐업률 등 발동 근거가 함께 표시된다.
>
> 검증 보고 탭은 H1·H2·H3 가설과 B1·B3 베이스라인 비교를 함께 제공한다. H1은 GRI 상위 20% 상권의 다음 분기 평균 폐업률이 하위 80%보다 높은지 검증하며, 실측 결과 1.084% vs 0.075%로 약 14.5배 차이(p≈8.0e-31, n=1,650)를 확인했다. H2는 순유출(`net_flow < 0`)이 큰 상권일수록 폐업률 상승과 양의 상관을 보이는지 Spearman 상관과 분위 비교로 노출한다. H3는 네트워크 고립도(`degree_centrality` 하위)가 높은 상권에서 매출 감소와 폐업률 상승이 동반되는지 분위별로 비교한다. B1은 공식 상권변화지표 및 기존 단순 임계값 대비 GRI가 추가 식별한 위험 상권 187건을, B3는 전기 대비 매출 감소율 모델 대비 추가로 포착한 위험 신호 231건을 비교 표로 제시한다. 이를 통해 spiceMap은 기존 지표를 보완해 흐름 기반 위험을 조기에 발견하고, 창업자·지원기관·자치구가 사전 대응할 수 있는 근거를 제공한다.
