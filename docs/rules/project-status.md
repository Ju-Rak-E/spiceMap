...
- **Week 5 (5/6~5/12)**: **D-Day (5/12)**. H1·H2·H3·B1 실측 수치 확정(2026-05-09), Railway 배포 인프라, `/api/barriers` centroid 좌표, **AI 창업 입지 분석(Startup Advisor) — `/api/advisor/industries`·`/api/advisor/startup` Claude API + `FounderPanelSections`·mock 데모**, **3D 뷰(자치구/상권 모드 + DistrictPin·CommerceColumn 픽토그램 + extrude-progress 애니메이션)**, **Module C non-OD barriers 풀구현**, **closure_rate 가중평균 fix(자치구×업종 매출) Supabase 적용 → H3 gap 0.746%p → 1.009%p (+35%)**, README·FR_Workflow 현행화. **D-1 보정(2026-05-11)**: OD flows `purpose` 파라미터 추가(PR #61) · Vercel tsc 빌드 복구(PR #60) · numpy 의존성 추가(PR #58) · Q1/Q2 estimates 백필 스크립트(PR #59) · 상권 추천 색상 변경(PR #57) · `frontend/vitest.config.ts` 분리. **D-Day 정비(2026-05-12)**: 공모전 제출 양식 사실 카드(PR #64 `docs/competition_submission_facts.md`) + 발표 핵심 기능 5선 치트시트(`docs/presentation_features.md`) + 활용 방안·기대 효과(`docs/Usagaeplan/usage_and_effects.md`) + Preview 디렉토리 정비(`docs/preview/` + `demo_storyline_gangnam_gwanak.md` + 페르소나 김미숙 PR #69) + OD flow 호버 알람카드(PR #66 `Map.tsx` +114줄) + barrier routes 자치구 정렬(`useBarrierRoutes`) + 파비콘 + **단절영역 flow 색상·폴리곤 라인 재수정(PR #67, 13개 layer/manager)** + **Railway 503 핫픽스 — type-map 자치구 fetch 직렬화(PR #68 `useCommerceData.ts:54-61`, Promise.all → for-await, 라이브 5/5 200 검증)**. 사용자 잔여: Hero shot PNG 5종·시연 영상·Vercel/Railway 실배포·`OPENROUTESERVICE_API_KEY`/`ANTHROPIC_API_KEY` 등록 결정·후속 옵션 B/C(백엔드 풀+워커, adm_comm_mapping JOIN) 발표 후 적용.
