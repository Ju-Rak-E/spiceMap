...
| 지표 | 정의 | NetworkX API |
|------|------|-------------|
| `in_degree` | 상권으로 들어오는 이동량 합 | `G.in_degree(weight="flow")` |
| `out_degree` | 상권에서 나가는 이동량 합 | `G.out_degree(weight="flow")` |
...
def build_commerce_flow_graph(
    ...
    for _, row in agg.iterrows():
        G.add_edge(row["origin_commerce"], row["dest_commerce"], flow=row["flow_count"])
    return G


def compute_degree_metrics(G: nx.DiGraph) -> pd.DataFrame:
    """그래프에서 degree 계열 지표 산출."""
    in_deg  = dict(G.in_degree(weight="flow"))
    out_deg = dict(G.out_degree(weight="flow"))
