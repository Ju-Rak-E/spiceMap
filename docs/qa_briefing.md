...
### Q3. "H2 결과를 왜 빈 값으로 두는가?" → **D-5 갱신: 실측 결과 — 예상 외 발견**

A. ~~함수와 테스트는 완성~~ → **D-5 (2026-05-07) 실측 산출 완료**. 결과:

- **r = -0.595, p ≈ 6.3 × 10⁻⁵, n = 39**
- **방향이 예상과 반대** (양의 상관 가설 → 음의 상관 실측)

**해석**: 흐름 단절은 보통 유동량 큰 hub 상권에서 발생. hub 는 안정적이라 폐업률이 낮다. 즉 selection bias 가능성.

**한계**:
- 표본 n=39 (flow_barriers 280페어 → 상권별 max 집계 시 매칭 39)
- closure_rate 자치구 매핑 한계 (H1·H3과 동일)
- 인과 방향 재고 필요 — 흐름 단절은 *결과* 일 수도

**v2 계획**: 표본 확장 (서울 전역 25 자치구) + 상권 단위 closure 직접 적재 후 재검증.

**발표에서 정직하게 보고**: "실측 결과 음의 상관으로 가설 FAIL — 표본/매핑 한계로 단정 어려움. v2 검증 필요" — 이는 *부정직한 fixture 보다 학술적/공공 신뢰성에 더 부합*.
